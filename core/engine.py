"""计算内核：科学计算 / 线性代数 / 绘图采样 / 单位 / 汇率 / 财务 / 日期 /
随机数 / 常数 / 单位类别。

合并自：core/engine.py + core/units.py + core/constants.py

变更历史（本轮）：
- 合并 `UNIT_CATEGORIES` / `categories()` / `get_category()` / `default_units()`
  （原 core.units）
- 合并 `MATH_CONSTANTS` / `PHYSICS_CONSTANTS` / `all_constants()` / `lookup()`
  （原 core.constants）
- 依赖 `core.symbols` 改为从 `core.state` 导入（合并后的符号存储）
- `_parse_lambda2` 公开为 `parse_lambda2`，保留旧别名

对外接口（向后兼容 core.engine）：
    basic_calc / basic_calc_smart / sci_eval / format_result
    sci_* (simplify/expand/factor/apart/trigsimp/solve/diff/integrate/
          limit/series/summation/product/dsolve/quad/minimize/linprog/
          complex_eval/solve_inequality/series_ext/steps_solve)
    matrix_unary / matrix_binary / matrix_random / matrix_solve_linear /
    matrix_inv_steps / matrix_from_csv_text / matrix_to_csv_text
    sample_cartesian / sample_polar / sample_parametric / sample_surface
    stats_calc / unit_convert / base_convert / base_convert_float
    ascii_convert / endian_swap / float_ieee754
    finance_loan / finance_compound
    date_diff / date_add / random_numbers / currency_convert / fetch_rates
    UNIT_CATEGORIES / categories / get_category / default_units
    MATH_CONSTANTS / PHYSICS_CONSTANTS / all_constants / lookup
    parse_lambda2
"""
...
from core.base import InputError, MathError, UnitError, NetworkError, log_warn
from core.state import (
    get_all as _sym_get_all,
    set_symbol as _sym_set,
    match_assignment as _sym_match_assign,
)
...
def default_units(key: str) -> tuple:
...
def _parse(expr, extra=None, base_locals=None):
    ...
    assignment = _sym_match_assign(s)
    ...
    if extra:
        local.update(extra)
    for name, val in _sym_get_all().items():
    ...
    try:
        ...
        raise InputError(f"表达式非法：{e}",
                         friendly_key="err_parse") from e
    except Exception as e:  # noqa: BLE001
        raise InputError(f"无法解析：{e}",
                         friendly_key="err_parse") from e
...
def _handle_assignment(assignment, base_locals=None):
    ...
    if len(syms) != len(args):
        raise InputError("函数参数名重复或非法",
                         friendly_key="err_parse")

    local = dict(base)
    for n, s in zip(args, syms):
        local[n] = s
    for k, v in _sym_get_all().items():
        if k not in local:
            local[k] = v
    ...
    _sym_set(name, lam, raw=raw_repr)
    return lam
...
def _format_scalar(obj, fmt, digits, sci, fraction, percent):
    try:
        is_number = (
...
def format_result(obj, fmt: str = "text", *,
    ...
    try:
        if isinstance(obj, dict):
            items = [(k,
                      _format_scalar(v, fmt, digits, sci, fraction,
                                     percent) or v)
                     for k, v in obj.items()]
            if fmt == "latex":
                return r",\quad ".join(
                    f"{sp.latex(k)} = "
                    f"{sp.latex(v) if not isinstance(v, str) else v}"
                    for k, v in items)
            if fmt == "unicode":
                return ",  ".join(
                    f"{k} = "
                    f"{sp.pretty(v, use_unicode=True) if not isinstance(v, str) else v}"
                    for k, v in items)
...
def basic_calc(expr, angle_mode="RAD"):
    ...
    try:
        ...
        raise MathError("除零错误",
                        friendly_key="err_div_zero") from ex
    ...
    if getattr(val, "is_number", False):
        ...
        if c is not None:
            if math.isnan(c.real) or math.isnan(c.imag):
                raise MathError("结果未定义 (NaN)",
                                friendly_key="err_nan")
...
def sci_solve_system(text: str, vars_str: str = "x, y"):
    ...
    names = [v.strip()
             for v in re.split(r"[,\s]+", vars_str) if v.strip()]
    if not names:
        names = ["x", "y"]
...
def sci_integrate(expr, var: str = "x", lower=None, upper=None):
    ...
    if lo and up:
        return sp.simplify(sp.integrate(
            e, (v, _parse(lo), _parse(up))))
    return sp.simplify(sp.integrate(e, v))


def sci_limit(expr, var: str = "x", point: str = "0",
              direction: str = "+"):
    v = sp.Symbol(var.strip() or "x")
...
def sci_series(expr, var: str = "x", point: str = "0",
               order: int = 6):
    v = sp.Symbol(var.strip() or "x")
...
def sci_summation(expr, var: str = "n", lower: str = "1",
                  upper: str = "10"):
    v = sp.Symbol(var.strip() or "n")
    return sp.summation(_parse(expr),
                        (v, _parse(lower), _parse(upper)))


def sci_product(expr, var: str = "n", lower: str = "1",
                upper: str = "10"):
    v = sp.Symbol(var.strip() or "n")
    return sp.product(_parse(expr),
                      (v, _parse(lower), _parse(upper)))
...
def _parse_matrix(text):
    ...
    try:
        if "[" not in s and (";" in s or "\n" in s):
            ...
            data = [[_parse(x) for x in r.split(",")]
                    for r in rows]
...
def matrix_unary(action: str, text: str, scalar=None):
    ...
    if action == "inv":
        if A.rows != A.cols:
            raise MathError("非方阵无法求逆",
                            friendly_key="err_math")
        if _is_singular(A):
            raise MathError("奇异矩阵不可逆",
                            friendly_key="err_math")
        try:
    ...
    if action == "eigenvects":
        if A.rows != A.cols:
            raise MathError("非方阵无特征向量",
                            friendly_key="err_math")
        return A.eigenvects()
    if action == "charpoly":
        if A.rows != A.cols:
            raise MathError("非方阵无特征多项式",
                            friendly_key="err_math")
        lam = sp.Symbol("lambda")
        return sp.simplify(A.charpoly(lam).as_expr())
    if action == "adjugate":
        if A.rows != A.cols:
            raise MathError("非方阵无伴随矩阵",
                            friendly_key="err_math")
    ...
    if action == "lu":
        ...
        return {"L": L, "U": U,
                "perm": (sp.Matrix([list(perm)])
                         if perm else sp.Matrix([[0]]))}
    if action == "qr":
        Q, R = A.QRdecomposition()
        return {"Q": Q, "R": R}
    if action == "exp":
        if A.rows != A.cols:
            raise MathError("非方阵无矩阵指数",
                            friendly_key="err_math")
...
def stats_calc(text):
    try:
        nums = [float(x) for x in
                re.split(r"[\s,;]+", str(text).strip()) if x]
...
def unit_convert(value, from_unit, to_unit):
    try:
        ...
        raise UnitError(f"单位换算失败：{e}",
                        friendly_key="err_unit") from e
...
def base_convert(value, from_base, to_base):
    ...
    if not (2 <= fb <= 36) or not (2 <= tb <= 36):
        raise InputError("进制范围应为 2~36",
                         friendly_key="err_base_range")
...
def base_convert_float(value, from_base, to_base, precision: int = 12):
    ...
    for i, c in enumerate(frac_part):
        d = _digits_val(c)
        if d < 0 or d >= fb:
            raise InputError(f"非法字符：{c!r}",
                             friendly_key="err_base_char")
        frac += d / (fb ** (i + 1))
...
def endian_swap(value, width: int = 32):
    ...
    return (swapped, be_bytes.hex().upper(), le_bytes.hex().upper())
...
def currency_convert(amount, from_cur, to_cur, rates):
    ...
    if fc not in rates or tc not in rates:
        raise NetworkError(
            f"未知货币：{fc if fc not in rates else tc}",
...
_PERCENT_PATTERNS = [
    (re.compile(r"^\s*([\d.]+)\s*%\s*off\s+([\d.]+)\s*$",
                re.I), "off"),
    (re.compile(r"^\s*([\d.]+)\s*%\s*on\s+([\d.]+)\s*$",
                re.I), "on"),
    (re.compile(r"^\s*([\d.]+)\s*%\s*of\s+([\d.]+)\s*$",
                re.I), "of"),
    (re.compile(r"^\s*tip\s+([\d.]+)\s*(?:%|percent)?\s+on\s+"
                r"([\d.]+)\s*$", re.I), "tip"),
    (re.compile(r"^\s*tax\s+([\d.]+)\s*(?:%|percent)?\s+on\s+"
                r"([\d.]+)\s*$", re.I), "tax"),
    (re.compile(r"^\s*discount\s+([\d.]+)\s*(?:%|percent)?\s+on\s+"
                r"([\d.]+)\s*$", re.I), "off"),
]
...
def _percent_special(s: str):
    ...
    if m:
        return (float(m.group(1)) / 100,
                f"{m.group(1)}% = {float(m.group(1))/100}")
    return None
    ...
    for pat, op in _INEQ_OPS:
        if pat.search(s):
            ...
            try:
                ...
                if rel is None:
                    return sp.solve(e, v)
                return sui(rel(e, 0), v, relational=False)
            except Exception as ex:  # noqa: BLE001
...
_SERIES_KINDS = {
    "taylor": "series", "fps": "fps", "laurent": "laurent",
...
def sci_dsolve(eq_str, func="y", var="x", ics=None):
    ...
    local[func] = y
    try:
        e = parse_expr(s, local_dict=local,
                       transformations=_TRANSFORMS)
    except Exception as ex:
...
def sci_minimize(expr, var="x", x0=0, method="BFGS"):
    ...
    try:
        res = _min(lambda a: float(f(a[0])), [float(x0)],
                   method=method)
    except Exception as ex:
...
def sci_linprog(params_json):
    try:
        p = (_json.loads(params_json)
             if isinstance(params_json, str) else params_json)
    except Exception as ex:
...
__all__ = [
    ...
    "format_result",
    # 基础 / 科学
    "basic_calc", "basic_calc_smart", "scientific_calc", "sci_eval",
    "sci_simplify", "sci_expand", "sci_factor", "sci_apart",
    "sci_trigsimp",
    "sci_solve", "sci_solve_system", "sci_solve_inequality",
    "sci_diff", "sci_integrate", "sci_limit", "sci_series",
    "sci_series_ext", "sci_summation", "sci_product",
    "sci_steps_solve", "sci_complex_eval",
    "solve_equation", "integrate_expr", "diff_expr", "limit_expr",
    # 矩阵
    "matrix_unary", "matrix_binary", "matrix_op",
    "matrix_random", "matrix_from_csv_text", "matrix_to_csv_text",
    "matrix_solve_linear", "matrix_inv_steps",
    # 采样
    "sample_cartesian", "sample_polar", "sample_parametric",
    "sample_surface", "plot_data", "parse_lambda2",
    # 统计 / 单位 / 进制
    "stats_calc", "unit_convert",
    "base_convert", "base_convert_float",
    "ascii_convert", "endian_swap", "float_ieee754",
    # 汇率 / 财务 / 日期
    "fetch_rates", "currency_convert",
    "finance_loan", "finance_compound",
    "date_diff", "date_add", "random_numbers",
    # ODE / 数值 / 优化
    "sci_dsolve", "sci_quad", "sci_minimize", "sci_linprog",
    # 概率桥
    "prob_fit", "prob_ci", "prob_anova", "prob_chi2_ind",
