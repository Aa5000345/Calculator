...
def b64_decode(text, url_safe=False):
    try:
        ...
        raise InputError(f"Base64 解码失败：{e}", friendly_key="err_input")
...
def hex_decode(text):
    ...
    try:
        ...
        raise InputError(f"Hex 解码失败：{e}", friendly_key="err_input")
    return data.decode("utf-8", errors="replace")
...
def hmac_text(text, key, algo="sha256", upper=True):
    ...
    if algo not in _HASHLIB_ALGOS:
        raise InputError(f"未知哈希算法：{algo}", friendly_key="err_input")
    h = hmac.new(str(key).encode("utf-8"),
...
def aes_gcm_encrypt(plaintext, key_hex):
    ...
    try:
        ...
        raise InputError(f"密钥必须是十六进制：{e}", friendly_key="err_input")
...
def aes_gcm_decrypt(cipher_hex, key_hex):
    ...
    try:
        ...
        raise InputError(f"十六进制解析失败：{e}", friendly_key="err_input")
    if len(key) not in (16, 24, 32):
        raise InputError("AES 密钥长度必须为 16/24/32 字节",
                         friendly_key="err_input")
    if len(data) < 13:
...
def rsa_generate(bits=2048):
    ...
    n = int(bits)
    if n < 1024:
        raise InputError("RSA 位数至少 1024", friendly_key="err_input")
    key = rsa.generate_private_key(public_exponent=65537, key_size=n)
    ...
    return {"private_key": priv_pem, "public_key": pub_pem, "bits": n}
...
def rsa_encrypt(plaintext, public_pem):
    ...
    try:
        pub = serialization.load_pem_public_key(str(public_pem).encode("ascii"))
    except Exception as e:
        raise InputError(f"公钥解析失败：{e}", friendly_key="err_input")
    ct = pub.encrypt(
...
def rsa_decrypt(cipher_hex, private_pem):
    ...
    try:
        ...
        raise InputError(f"解析失败：{e}", friendly_key="err_input")
    try:
        pt = priv.decrypt(ct, padding.OAEP(
            mgf=padding.MGF1(algorithm=hashes.SHA256()),
            algorithm=hashes.SHA256(), label=None))
    except Exception as e:
        raise InputError(f"解密失败：{e}", friendly_key="err_input")
    return pt.decode("utf-8", errors="replace")
...
def totp(secret_b32, digits=6, period=30, at_time=None):
    s = str(secret_b32).strip().upper().replace(" ", "").replace("-", "")
    pad = (-len(s)) % 8
    s += "=" * pad
    try:
        key = base64.b32decode(s)
    except Exception as e:
        raise InputError(f"Base32 密钥解析失败：{e}", friendly_key="err_input")
    t = int(at_time) if at_time is not None else int(time.time())
    counter = t // int(period)
    code = _hotp(key, counter, int(digits))
    return {
        "code": code, "counter": counter, "period": int(period),
        "remaining": int(period) - (t % int(period)),
    }


def gen_totp_secret(bytes_len=20):
    return base64.b32encode(secrets.token_bytes(int(bytes_len))).decode("ascii")
...
def password_strength(pw):
    ...
    if any(not c.isalnum() for c in s): pool += 33
    entropy = len(s) * math.log2(pool) if pool > 1 else 0
    if entropy < 28: level = "very_weak"
    elif entropy < 40: level = "weak"
    elif entropy < 60: level = "medium"
    elif entropy < 80: level = "strong"
    else: level = "very_strong"
    return {"length": len(s), "pool": pool,
            "entropy": round(entropy, 2), "level": level}