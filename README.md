...
> 一款**键盘驱动、随手可用**的桌面多功能计算器，把 20+ 个常用工具收进一个窗口。  
...
- **23 个面板**，按 8 个分组组织：基础 / 转换 / 数据 / 数学 / 财务 / 工具 / 生产力 / 系统
- **命令面板（`Ctrl+K`）**：`= 2+2` 直接算、`>theme` 切主题、任意文本搜历史
- **变量与函数定义**：`x = 5`、`f(x) = x^2 + 1`，跨会话持久化
- **单位感知计算**：`1 km + 500 m` → `1.5 kilometer`，结果一键送到单位面板
- **键盘驱动**：`+` 不用 Shift，`Esc` 清空，`↑/↓` 召回历史
...
> 若使用 Python 3.13 / 3.14 等新版本，个别可选依赖（`lunardate`、`astral`）可能暂无 wheel，可先跳过，相关功能会给出友好提示。
...
- "变量"标签页可查看、插入、删除、清空
- `x`、`y` 这类常见符号默认作为**自由符号**，只有被显式赋值后才参与计算
...
100 W * 2 h               →  0.2 kilowatt_hour
2 ft + 6 in               →  0.762 meter
```

- 结果通过右键菜单 **"发送到单位面板"** 一键送到单位换算面板继续处理
- 单位支持来自 `pint`，覆盖长度、质量、温度、速度、压力、能量、数据、时间、角度等
...
│   └── panels/                # 23 个功能面板
│       ├── registry.py        # 面板注册表
│       ├── _common.py         # ResultView / CollapsibleGroup
│       └── ...
└── plugins/
    └── rates/                 # 汇率源插件目录
```
...
4. 如需语言包键值，在 `config/i18n/zh_CN.json` 与 `en_US.json` 里补充。
...
```bash
# 单独 import 某个模块，看更清晰的 traceback
python -c "import ui.panels.finance"
python -c "import core.engine"
```
...
**Q：数据保存在哪？想清空所有数据？**  
A：删除 `~/.multicalc/` 目录即可（会同时清空历史、变量、片段、日志）。
