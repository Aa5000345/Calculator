"""加密工具面板：编码 / 经典密码 / 哈希-HMAC / AES / RSA / TOTP / 密码强度。"""
...
from PySide6.QtWidgets import (
    ...
    QHBoxLayout, QFormLayout, QComboBox, QSpinBox, QCheckBox, QTabWidget,
...
class CryptoPanel(CalcPanel):
    ...
    def __init__(self, settings, i18n, history):
        ...
        tabs = QTabWidget()
        tabs.addTab(self._build_encode_tab(), i18n.t("crypto_encode", "Encode"))
        tabs.addTab(self._build_classic_tab(), i18n.t("crypto_classic", "Classic"))
        tabs.addTab(self._build_hash_tab(), i18n.t("crypto_hash", "Hash / HMAC"))
        tabs.addTab(self._build_aes_tab(), i18n.t("crypto_aes", "AES"))
        tabs.addTab(self._build_rsa_tab(), i18n.t("crypto_rsa", "RSA"))
        tabs.addTab(self._build_otp_tab(), i18n.t("crypto_otp", "TOTP"))
        tabs.addTab(self._build_pw_tab(), i18n.t("crypto_pw", "Password"))
    ...
    def _err(self, e):
        self.result.setPlainText(friendly_error(self.i18n, e, "crypto"))
    ...
    def _build_encode_tab(self):
        ...
        b_enc = QPushButton(self.i18n.t("encode", "Encode")); b_enc.clicked.connect(self._do_encode)
        b_dec = QPushButton(self.i18n.t("decode", "Decode")); b_dec.clicked.connect(self._do_decode)
        ...
        row = QHBoxLayout(); row.addWidget(b_enc); row.addWidget(b_dec)
    ...
    def _do_encode(self):
        try:
            ...
            if kind == "base64":
                ...
                return
            self._show({"kind": kind, "output": r}, tag=f"enc-{kind}")
    ...
    def _do_decode(self):
        try:
            ...
            if kind == "base64":
                ...
                return
            self._show({"kind": kind, "output": r}, tag=f"dec-{kind}")
        except Exception as e:
            self._err(e)
    ...
    def _build_classic_tab(self):
        ...
        self.cls_shift = QSpinBox(); self.cls_shift.setRange(-25, 25); self.cls_shift.setValue(3)
        b = QPushButton(self.i18n.t("apply", "Apply")); b.clicked.connect(self._do_classic)

        w = QWidget(); f = QFormLayout(w)
        f.addRow(QLabel(self.i18n.t("input", "Input")), self.cls_in)
        f.addRow(QLabel(self.i18n.t("kind", "Kind")), self.cls_kind)
    ...
    def _do_classic(self):
        try:
            ...
            self._show({"kind": kind, "output": r}, tag=f"classic-{kind}")
        except Exception as e:
            self._err(e)
    ...
    def _build_hash_tab(self):
        self.h_in = QPlainTextEdit("hello world"); self.h_in.setFixedHeight(70)
        self.h_algo = QComboBox()
        for k in ("md5", "sha1", "sha224", "sha256", "sha384", "sha512",
        ...
        self.h_upper = QCheckBox(self.i18n.t("uppercase", "Uppercase"))
        b = QPushButton("Hash"); b.clicked.connect(self._do_hash)

        w = QWidget(); f = QFormLayout(w)
        f.addRow(QLabel(self.i18n.t("input", "Input")), self.h_in)
    ...
    def _do_hash(self):
        try:
            ...
            key = self.h_key.text()
            if key:
                r = ct.hmac_text(text, key, algo, self.h_upper.isChecked())
            else:
                r = ct.hash_text(text, algo, self.h_upper.isChecked())
    ...
    def _build_aes_tab(self):
        self.aes_plain = QPlainTextEdit("Secret message"); self.aes_plain.setFixedHeight(70)
        self.aes_cipher = QPlainTextEdit(""); self.aes_cipher.setFixedHeight(70)
        self.aes_key = QLineEdit(ct.gen_aes_key(256))
        self.aes_bits = QSpinBox(); self.aes_bits.setRange(128, 256); self.aes_bits.setSingleStep(64)
        self.aes_bits.setValue(256)
        b_gen = QPushButton(self.i18n.t("gen_key", "Generate key")); b_gen.clicked.connect(self._aes_gen_key)
        b_enc = QPushButton(self.i18n.t("encode", "Encrypt")); b_enc.clicked.connect(self._aes_enc)
        b_dec = QPushButton(self.i18n.t("decode", "Decrypt")); b_dec.clicked.connect(self._aes_dec)
    ...
    def _aes_gen_key(self):
        try:
            self.aes_key.setText(ct.gen_aes_key(self.aes_bits.value()))
        except Exception as e:
            self._err(e)

    def _aes_enc(self):
        try:
            r = ct.aes_gcm_encrypt(self.aes_plain.toPlainText(), self.aes_key.text())
    ...
    def _aes_dec(self):
        try:
            r = ct.aes_gcm_decrypt(self.aes_cipher.toPlainText(), self.aes_key.text())
    ...
    def _build_rsa_tab(self):
        self.rsa_priv = QPlainTextEdit(""); self.rsa_priv.setFixedHeight(100)
        self.rsa_pub = QPlainTextEdit(""); self.rsa_pub.setFixedHeight(100)
        self.rsa_bits = QSpinBox(); self.rsa_bits.setRange(1024, 4096); self.rsa_bits.setSingleStep(512)
        ...
        v.addWidget(QLabel("Plaintext")); v.addWidget(self.rsa_in)
        v.addWidget(QLabel("Cipher (hex)")); v.addWidget(self.rsa_ct)
        row2 = QHBoxLayout(); row2.addWidget(b_enc); row2.addWidget(b_dec)
    ...
    def _rsa_enc(self):
        try:
            r = ct.rsa_encrypt(self.rsa_in.toPlainText(), self.rsa_pub.toPlainText())
            self.rsa_ct.setPlainText(r)
            self._show({"cipher": r[:120] + "..." if len(r) > 120 else r}, tag="rsa-enc")
        except Exception as e:
            self._err(e)

    def _rsa_dec(self):
        try:
            r = ct.rsa_decrypt(self.rsa_ct.toPlainText(), self.rsa_priv.toPlainText())
    ...
    def _build_otp_tab(self):
        ...
        self.otp_digits = QSpinBox(); self.otp_digits.setRange(6, 10); self.otp_digits.setValue(6)
        self.otp_period = QSpinBox(); self.otp_period.setRange(10, 120); self.otp_period.setValue(30)
        b_gen = QPushButton(self.i18n.t("gen_key", "New secret")); b_gen.clicked.connect(self._otp_gen)
        b_calc = QPushButton(self.i18n.t("calc", "Generate")); b_calc.clicked.connect(self._otp_calc)
        ...
        row = QHBoxLayout(); row.addWidget(b_gen); row.addWidget(b_calc)
        f.addRow(row)
        return w
    ...
    def _otp_calc(self):
        try:
            r = ct.totp(self.otp_secret.text(), self.otp_digits.value(),
    ...
    def _build_pw_tab(self):
        ...
        b = QPushButton(self.i18n.t("calc", "Analyze")); b.clicked.connect(self._pw_strength)
        w = QWidget(); f = QFormLayout(w)
