...
class BitsPanel(CalcPanel):
    ...
    def __init__(self, settings, i18n, history):
        ...
        self.bitmap_label.setAlignment(Qt.AlignLeft | Qt.AlignTop)

        tabs = QTabWidget()
        tabs.addTab(self._build_op_tab(), i18n.t("bit_op", "Operations"))
        tabs.addTab(self._build_crc_tab(), "CRC")
        tabs.addTab(self._build_hash_tab(), "Hash")
        tabs.addTab(self._build_bitmap_tab(), i18n.t("bitmap", "Bitmap"))
    ...
    def _build_op_tab(self):
        self.a = QLineEdit("-1"); self.b = QLineEdit("5")
        self.width = QSpinBox(); self.width.setRange(4, 128); self.width.setValue(32)
        self.op = QComboBox()
        self.op.addItems(["and", "or", "xor", "not", "shl", "shr"])
        b_unsigned = QPushButton(self.i18n.t("to_unsigned", "To unsigned"))
        b_signed = QPushButton(self.i18n.t("to_signed", "To signed"))
        b_comp = QPushButton(self.i18n.t("twos_complement", "Two's complement"))
        ...
        f.addRow(QLabel(self.i18n.t("bit_width", "Width")), self.width)
        f.addRow(QLabel(self.i18n.t("bit_op", "Op")), self.op)
        row = QHBoxLayout()
        for btn in (b_unsigned, b_signed, b_comp): row.addWidget(btn)
        f.addRow(row); f.addRow(b_op)
        return w

    def _show(self, obj, tag="bits"):
        s = json.dumps(obj, ensure_ascii=False, indent=2, default=str)
        self.result.setPlainText(s)
        self.add_history(self.op.currentText() if hasattr(self, "op") else tag,
                         s, module=tag)

    def do_unsigned(self):
        try:
            v = bitmod.to_unsigned(int(self.a.text()), self.width.value())
            self._show({"unsigned": v, "bin": bitmod.bit_bin(v, self.width.value()),
    ...
    def do_signed(self):
        try:
            v = bitmod.to_signed(int(self.a.text()), self.width.value())
            self._show({"signed": v})
        except Exception as e:
            self.result.setPlainText(str(e))
    ...
    def do_op(self):
        try:
            w = self.width.value()
            r = bitmod.bit_op(int(self.a.text()), int(self.b.text()),
                              self.op.currentText(), w)
            self._show({"result": r, "bin": format(r, f"0{w}b"),
    ...
    def _build_crc_tab(self):
        ...
        self.crc_kind.addItems(["crc32", "crc16_ccitt", "crc16_modbus"])
        b = QPushButton("CRC"); b.clicked.connect(self._do_crc)
        w = QWidget(); f = QFormLayout(w)
    ...
    def _do_crc(self):
        try:
            ...
            self.result.setPlainText(s)
            self.add_history(f"{kind}:{text[:30]}", s, module="bits-crc")
        except Exception as e:
            self.result.setPlainText(friendly_error(self.i18n, e, "bits"))

    def _build_hash_tab(self):
        self.hash_text = QPlainTextEdit("hello world"); self.hash_text.setFixedHeight(80)
        ...
        self.hash_upper = QCheckBox(self.i18n.t("uppercase", "Uppercase"))
    ...
    def _do_hash(self):
        try:
            r = bit_ext.hash_text(self.hash_text.toPlainText(),
                                  self.hash_algo.currentText(),
                                  self.hash_upper.isChecked())
            self.result.setPlainText(json.dumps(r, ensure_ascii=False, indent=2))
        except Exception as e:
            self.result.setPlainText(friendly_error(self.i18n, e, "bits"))

    def _build_bitmap_tab(self):
        self.bm_value = QLineEdit("0xDEADBEEF")
        self.bm_width = QSpinBox(); self.bm_width.setRange(4, 4096); self.bm_width.setValue(32)
        b = QPushButton(self.i18n.t("render", "Render")); b.clicked.connect(self._do_bitmap)
        w = QWidget(); f = QFormLayout(w)
        f.addRow(QLabel("Value (int/hex)"), self.bm_value)
        f.addRow(QLabel("Width"), self.bm_width)
        f.addRow(b); f.addRow(self.bitmap_label)
        return w
    ...
    def _do_bitmap(self):
        try:
            ...
            s = json.dumps({"width": info["width"], "cols": info["cols"],
                            "rows": info["rows"],
                            "bits_first_row": info["bits"][0][:32]},
                           ensure_ascii=False, indent=2)
            self.result.setPlainText(s)
            self.add_history(raw, s[:200], module="bits-bitmap")
        except Exception as e:
            self.result.setPlainText(friendly_error(self.i18n, e, "bits"))

    def _render_bitmap(self, info):
        cols = info["cols"]; rows = info["rows"]; cell = 8
        img = QImage(cols * cell, rows * cell, QImage.Format_RGB32)
        img.fill(QColor("#111111"))
        for r in range(rows):
            for c in range(cols):
                color = QColor("#00e0ff") if info["bits"][r][c] else QColor("#222222")
                for dy in range(cell):
                    for dx in range(cell):
                        img.setPixelColor(c * cell + dx, r * cell + dy, color)
